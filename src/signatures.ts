...
export function getSetVaultDetailsSignatureTypedData(
  data: types.RestRequestSetVaultDetails,
  contractAddress: string,
  chainId: number,
  sandbox: boolean,
): Parameters<SignTypedData> {
...
export function getSetVaultXConnectionSignatureTypedData(
  data: types.RestRequestSetVaultXConnection,
  contractAddress: string,
  chainId: number,
  sandbox: boolean,
): Parameters<SignTypedData> {
  ...
  return [
    ...
    } satisfies Record<keyof types.RestRequestSetVaultXConnection, unknown>,
  ];
}

export function getRemoveVaultXConnectionSignatureTypedData(
  data: types.RestRequestRemoveVaultXConnection,
  contractAddress: string,
  chainId: number,
  sandbox: boolean,
): Parameters<SignTypedData> {
  ...
  return [
    ...
    },
    {
      ...
      wallet: data.wallet,
      manager: data.manager,
    } satisfies Record<keyof types.RestRequestRemoveVaultXConnection, unknown>,
  ];
}
...
export const getWithdrawalFromManagedAccountContractStructSignatureTypedData =
  async (
    ...
    if (
      Number(data.withdrawalType) === ManagedAccountWithdrawalType.byQuantity
    ) {
      return getWithdrawalFromManagedAccountByQuantitySignatureTypedData(
        {
          ...
          quantity: pipToDecimal(
            BigInt(data.withdrawalByQuantity.grossQuantity),
          ),
          ...
          managedAccountProvider: await ethers.resolveAddress(
            data.withdrawalByQuantity.managedAccountProvider,
          ),
          managedAccountProviderPayload: ethers.hexlify(
            data.withdrawalByQuantity.managedAccountProviderPayload,
          ),
    ...
    if (Number(data.withdrawalType) === ManagedAccountWithdrawalType.byShares) {
      return getWithdrawalFromManagedAccountBySharesSignatureTypedData(
        {
          ...
          managedAccountProvider: await ethers.resolveAddress(
            data.withdrawalByShares.managedAccountProvider,
          ),
          managedAccountProviderPayload: ethers.hexlify(
            data.withdrawalByShares.managedAccountProviderPayload,
          ),
          bridgeAdapterAddress: await ethers.resolveAddress(
...
export const getWithdrawalFromManagedAccountByQuantitySignatureTypedData = (
  ...
  return [
    ...
    },
    {
      ...
      managedAccountProvider: data.managedAccountProvider,
      managedAccountProviderPayload: data.managedAccountProviderPayload,
...
export function getTransferSignatureTypedData(
  ...
  return [
    ...
    },
    {
      ...
      destinationWallet: data.targetWallet,
      quantity: data.quantity,
    },
  ];
}
...
export function uint128ToUuid(uint: ethers.BigNumberish): string {
  return uuidStringify(ethers.getBytes(ethers.toBeHex(uint, 16)));
}
...
export function uuidToUint128(uuid: string): bigint {
