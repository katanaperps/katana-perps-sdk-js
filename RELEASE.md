...
- When adding a commit to `beta` or `alpha`, the bot will check if a PR already exists to transition the pre-release to the next stage and open one if it doesn't exist
...
> **NOTE**: There is a bug in the generator at times when the `generateNotes` step will fail silently, causing a release to not be created. You can see an example by looking at the `Semantic Release` step in github actions where it simply stops silently after it gets to the `generateNotes`.  
